...
def cip_finder_new(
    ...
    print(
        f"{anion.split()[-1]}–{cation.split()[-1]} "
        f"distance cutoff (Å): {r_cut}"
    )
    ...
    for idx in range(start, end):
        ...
        # Reference atoms used for the DBSCAN clustering
        ions = cations + anions
        ...
        for cluster_id in unique_clusters:
            ...
            # Selected cation/anion reference atoms
            cluster_atoms = ions[mask]

            # Recover all atoms belonging to the ionic residues
            full_cluster = cluster_atoms.residues.atoms

            cluster_charges = (
                full_cluster.charges.copy()
            )

            total_charge = cluster_charges.sum()

            # Keep only neutral ionic aggregates
            if abs(total_charge) > 1e-4:
                continue

            # Reconstruct the entire neutral cluster across PBC
            cluster_positions = unwrap_cluster_pbc(
            ...
            show_unwrapped_cluster(
                frame=idx,
                cluster_id=cluster_id,
                cluster_atoms=cluster_atoms,
                full_cluster=full_cluster,
                unwrapped_positions=cluster_positions,
                box=box,
            )
            cluster_types = (
                full_cluster.types.copy()
            )

            cluster_indices = (
                full_cluster.indices.copy()
            )

            # Number of ionic reference sites in this cluster
            n_ions = mask.sum()
...
def unwrap_cluster_pbc_new(
    ...
    # Positions of the ionic reference atoms used by DBSCAN
    ref_positions = cluster_atoms.positions.copy()

    n_ions = len(cluster_atoms)

    if n_ions == 0:
        raise ValueError(
            "The cluster contains no reference atoms."
        )

    # Each selected reference atom must belong to a different
    # ionic residue.
    ref_resindices = cluster_atoms.resindices.copy()

    if len(np.unique(ref_resindices)) != n_ions:
        raise ValueError(
            "More than one reference atom was found for "
            "the same ionic residue."
        )

    # ==========================================================
    # Minimum-image distances between ionic reference atoms
    #
    # delta[i, j] = position[j] - position[i]
    # ==========================================================
    delta = (
        ref_positions[None, :, :]
        - ref_positions[:, None, :]
    ...
    # Same connectivity criterion used by DBSCAN
    connected = (
        distances <= r_cut
    )
    ...
    # ==========================================================
    # Reconstruct the positions of the ionic reference atoms
    # ==========================================================
    unwrapped_ref_positions = np.zeros_like(
        ref_positions
    )
    ...
    # The first ion is used as the starting reference.
    unwrapped_ref_positions[0] = (
        ref_positions[0]
    )
    ...
    while queue:
        ...
        for j in neighbors:
            ...
            # Place ion j relative to the already reconstructed
            # position of ion i.
            unwrapped_ref_positions[j] = (
                unwrapped_ref_positions[i]
                + delta[i, j]
            )
    ...
    # A DBSCAN cluster should be connected using the same cutoff.
    if not np.all(visited):

        missing = np.where(~visited)[0]

        raise ValueError(
            "Could not reconstruct the complete ionic cluster. "
            f"Disconnected reference atoms: {missing.tolist()}"
        )

    # ==========================================================
    # Reconstruct every complete ionic residue
    # ==========================================================

    # Output must preserve the atom order of full_cluster.
    unwrapped_positions = np.empty_like(
        full_cluster.positions
    )

    # Map global atom index -> position inside full_cluster
    local_index = {
        int(atom_index): i
        for i, atom_index
        in enumerate(full_cluster.indices)
    }

    for ion_index, resindex in enumerate(
        ref_resindices
    ):

        # All atoms belonging to this ionic residue
        residue_atoms = full_cluster[
            full_cluster.resindices == resindex
        ]

        # Wrapped position of the reference atom for this residue
        wrapped_ref = ref_positions[
            ion_index
        ]

        # Reconstructed position of that same reference atom
        unwrapped_ref = unwrapped_ref_positions[
            ion_index
        ]

        # ------------------------------------------------------
        # Make the individual residue whole relative to its
        # own reference atom.
        # ------------------------------------------------------
        residue_delta = (
        ...
        # Store positions while preserving full_cluster ordering.
        indices_in_full_cluster = np.array(
            [
                local_index[int(atom_index)]
                for atom_index
                in residue_atoms.indices
            ]
        )

        unwrapped_positions[
            indices_in_full_cluster
        ] = residue_positions
...
def show_unwrapped_cluster(
    ...
    """
    Print wrapped and unwrapped positions for clusters crossing PBC.
    """

    box = np.asarray(box, dtype=float)
    wrapped_positions = cluster_atoms.positions.copy()

    # A large coordinate span suggests that the cluster crosses PBC.
    wrapped_span = np.ptp(wrapped_positions, axis=0)

    if not np.any(wrapped_span > box / 2.0):
        return

    # Map each atom index to its position in full_cluster.
    local_indices = {
        atom_index: i
        for i, atom_index in enumerate(full_cluster.indices)
    }

    print(
        f"\nPBC cluster found: "
        f"frame={frame}, cluster={cluster_id}"
    )

    print(f"Box: {box}")
    print(f"Wrapped ion span: {wrapped_span}")

    for atom in cluster_atoms:

        local_index = local_indices[atom.index]

        wrapped = atom.position
        unwrapped = unwrapped_positions[local_index]

        print(
            f"{atom.resname:>4s} "
            f"{atom.name:>4s} "
            f"index={atom.index:<6d} "
            f"wrapped=("
            f"{wrapped[0]:8.3f}, "
            f"{wrapped[1]:8.3f}, "
            f"{wrapped[2]:8.3f}) "
    ...
    unwrapped_reference_positions = np.array(
        [
            unwrapped_positions[local_indices[atom.index]]
            for atom in cluster_atoms
        ]
    )

    unwrapped_span = np.ptp(
        unwrapped_reference_positions,
        axis=0,
    )

    print(f"Unwrapped ion span: {unwrapped_span}")